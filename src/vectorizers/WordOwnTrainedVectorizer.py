...
class WordOwnTrainedVectorizer(Vectorizer):
    ...
    def _vectorize(self, X):
        ...
        for x in X:
            text_vector = np.array(
                [np.zeros(self.wv_length) if word not in self._model.wv.vocab else self._model.wv[word] for word in
                 x.split(' ')])
    ...
    def save(self, save_file=None, pca_save_file=None):
        ...
        if not pca_save_file:
            pca_save_file = WOTV_MODEL_DIR.replace('{}', self.short_name + '-pca')
    ...
    def load(self, load_file=None, pca_load_file=None):
        ...
        if not pca_load_file:
            pca_load_file = WOTV_MODEL_DIR.replace('{}', self.short_name + '-pca')
